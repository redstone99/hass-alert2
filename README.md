...
Alert2 is a [Home Assistant](https://www.home-assistant.io/) component that supports alerting and sending notificaitons based on conditions and events. It's a retake on the original [Alert](https://www.home-assistant.io/integrations/alert/) integration.
...
- **Persistent notification details**. In your HA dashbaord, you can view past alert firings as well as the message text sent in notificaitons.
...
Each alert can specify a template message that is evaluated each time the alert fires. That message is sent out with notifications and also is viewable in the frontend UI.

Each alert maintains a bit indicating whether it has been ack'd or not.  That bit is reset each time the alert fires. Acking is done by clicking a button in the UI (described below). Acking stops reminder notifications (see below) and is indicated visually in the UI.
...
The text of a notification can include a custom message with details. It also includes some basic information such as how long the alert has been firing (for condition alerts) as well as how many times it has fired since the last notificaiton. Usually the notification text will be simplie, like:
...
| `name` | string | part of the entity name of the alert. The entity name of an alert is `alert2.{domain}_{name}`. `name` is typically the particular fault occuring (e.g., open_too_long) |
...
All parameters are required, except `condition` and `trigger`, which should have only one defined. The reason that alert names are split into `domain` and `name` is partly for semantic clarity and also for future mangement features, like grouping alerts by domain.
...
The card lists recently active alerts, as well as snoozed or disabled alerts.  A slider at the top of the card controls the time window covered. Each line shows the status of the alert, including when it last fired, how many times it fired since the last notificaiton, and whether it has been acked, snoozed or disabled.  Each alert will show an "ACK" button if it hasn't been acked already. The button "ACK ALL" will ack all alerts, not just the ones displayed.
...
The `alert2` module also offers a `create_task()` method to create tasks. It's similar to `hass.async_create_task` ecept it also `report()`s uncaught exceptions - so your task doesn't die silently.  Example usage:
